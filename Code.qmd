...
# Step 5: Plot the combined data with box plots for all three periods (with improved visibility)
ggplot(data_long, aes(x = Regulation_Period, y = Deaths_per_100k, fill = Sex)) +
  ...
  geom_text(data = median_values, aes(label = round(Median_Deaths, 1), y = Median_Deaths + 5),  # Annotate medians, adjust position
            position = position_dodge(0.75), size = 4, color = "black", vjust = -0.5) +  # Use black color for better contrast
  scale_fill_brewer(palette = "Set1") +  # Use a color palette for professional aesthetics
  labs(title = "Lung Cancer Deaths per 100,000: Pre, Post, and Strict Smoking Regulations",
       ...
       fill = "Sex") +
  ylim(0, 50) +  # Adjust y-axis limits to provide more room for annotations
...
# Step 1: Summarize the data by calculating total deaths for each country
...
# Step 5: Create the stacked bar chart for top 10 countries
top_10_plot <- ggplot(data_long, 
  ...
  labs(
    ...
    x = "Country",
    y = "Number of Deaths",
    fill = "Gender"  # Renamed the legend title
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
    ...
    legend.position = "top",  # Move legend closer to the plot
    legend.text = element_text(size = 12),  # Adjust the size of the legend labels
    legend.title = element_text(size = 14, face = "bold"),  # Enhance legend title
    panel.grid.major = element_line(linewidth = 0.5, linetype = 'dashed', color = "gray80")
  )

# Step 6: Print the top 10 countries plot
print(top_10_plot)
...
# Check the data summary to ensure the average is calculated
...
# Rendering the male map to check if it is generated correctly
print("Rendering Male Lung Cancer Deaths Map:")
print(male_map)

# Convert the male map to an interactive plotly object
interactive_map_male <- ggplotly(male_map, tooltip = c("text"))  # Specify tooltip to show country names

# Render the interactive plot
interactive_map_male  # Just call the variable to render
